...
class class_DR00 extends class_top {
  ...
  var $m_arr_current_stock = array();
    ...
    function DR05() //  by junho
    {
        //$this->show_wait();
        global $connect;
        global $template, $page;
        global $start_date, $end_date, $confirm, $number;

        //$today = Date("Y-m-d");
        $today = "2014-12-12";
        //$today = "2014-06-04";
        //echo "출력 date:" . $today . "/no: " . $number . "<br>";

        $query = "select seq_list 
                    from print_number 
                    where crdate='$today' and no=$number";
        $result = mysql_query( $query , $connect );
        $data   = mysql_fetch_assoc( $result );
        
        $query = "select seq, pack from orders where seq in ( $data[seq_list] )";
        $result = mysql_query( $query, $connect );
        ...
        while ( $data = mysql_fetch_assoc( $result ) )
        {
            ...
            if( $data[pack] )   //  합포이면 같은 합포번호 검사 후 추가
            {
                foreach( $real_seq_list as $key => $arr )  
                {
                    if( $arr[pack] == $data[pack] )
                    {
                        $is_matched = true;
                        $real_seq_list[$key][seq][] = $data[seq];  
                    }
                    else continue;
                }
                if( !$is_matched )
                {
                    unset($arr_manage_num[seq]);
                    $arr_manage_num[pack] = $data[pack];
                    $arr_manage_num[seq][] = $data[seq];
                    $real_seq_list[] = $arr_manage_num;
                }
            }
            else    //  단일 상품이면 바로 추가
            {
        ...
        $total_order_count = count( $real_seq_list );   //  전체 송장 갯수
        
        $this->output_products( $real_seq_list, $total_order_count );
        //$this->hide_wait();
    }

    function output_products( $real_seq_list, $total_order_count )
    {
        $print_packs = "<style type=text/css>
                        .title{
        ...
        $count = 0; // 송장 인덱스와 분할 인쇄를 하는데 쓰인다. ( 송장 하나마다 1씩 증가 )
        foreach($real_seq_list as $arr)  
	    {
	        $count++;
	        if($arr[pack])     // 합포이면 pack으로 검사
            {
                $seq_list = "";
	            foreach( $arr[seq] as $seq )   
                {
                    $seq_list .= $seq_list ? "," : "";
                    $seq_list .= $seq;
                }
                $query_order = "select pack, seq, recv_name, recv_address, collect_date, shop_id, trans_no
                                from orders 
                                where pack=$arr[pack] 
                                limit 1";
                $query_product = "select a.product_id, a.qty, a.order_cs, b.name, b.img_500, b.options, b.shop_price 
                                  from order_products a, products b 
                                  where a.product_id=b.product_id and a.order_seq 
                                  in ( $seq_list )";
                $print_packs .= $this->make_table( $query_order, $query_product, $count, $total_order_count );
            }
            else    //  단일 주문이면 seq로 검사
            {
                foreach( $arr[seq] as $seq )
	            {
                    $query_order = "select pack, seq, recv_name, recv_address, collect_date, shop_id, trans_no 
	                ...
	                $query_product = "select a.product_id, a.qty, a.order_cs, b.name, b.img_500, b.options, b.shop_price 
	                                from order_products a, products b 
	                                where a.product_id=b.product_id and a.order_seq=$seq";
                    $print_packs .= $this->make_table( $query_order, $query_product, $count, $total_order_count );
                }
            }
        }        
    ...
    function make_table( $query_order, $query_product, $count, $total_order_count )
    {
        global $connect;
        ...
        {
            $shop_name = class_shop::get_shop_name( $data_order['shop_id'] );
            
            if($data_order[pack])    //  pack이면 관리번호는 pack
            {
                $manage_num = $data_order[pack];
                $manage_string = " / ( 합포 )";
            }
            
            else    //  seq이면 관리번호는 seq
            {
                $manage_num = $data_order[seq];
                $manage_string = " / ( 단일 주문 )";
            }
            
            $print_packs .= "<div ";
            if( $count > 1 )    $print_packs .= "class=page";  
            $print_packs .= "><br><table class=product>";   //  모든 상단 테이블 height='30'
            $print_packs .= "<tr><td class=title width='100'>인덱스</td><td width='400'>&nbsp;" . $count . " / " . $total_order_count . "</td></tr>";
            $print_packs .= "<tr><td class=title width='100'>관리번호</td><td width='400'>&nbsp;" . $manage_num . $manage_string . "</td></tr>";
            $print_packs .= "<tr><td class=title width='100'>송장번호</td><td width='400'>&nbsp;" . $data_order[trans_no] . "</td></tr>";
            $print_packs .= "<tr><td class=title width='100'>출력일자</td><td>&nbsp;" . $time . "</td></tr>";
            $print_packs .= "<tr><td class=title width='100'>발주일자</td><td>&nbsp;" . $data_order[collect_date] . "</td></tr>";  
            $print_packs .= "<tr><td class=title width='100'>고객명</td><td>&nbsp;" . $data_order[recv_name] . "</td></tr>";  
            $print_packs .= "<tr><td class=title width='100'>주소</td><td width='600'>&nbsp;" . $data_order[recv_address] . "</td></tr>";
            $print_packs .= "<tr><td class=title width='100'>판매처</td><td>&nbsp;" . $shop_name . "</td></tr></table><br>"; 
            
            $print_packs .= "<table class=product>  
                            <tr><td class=title width='100' >이미지</td>
                            <td class=title width='170' min-width='170' >상품명</td>
                            <td class=title width='200' >옵션</td>
                            <td class=title width='50' >개수</td>
                            <td class=title width='70' >금액</td>
                            <td class=title width='130' >상태</td></tr>"; //  이미지 height='30'
                            
            $result_product = mysql_query( $query_product, $connect );
            
            while ( $data_product = mysql_fetch_assoc( $result_product ) )
            {
                $product_name = $data_product[name]; 
                $product_img = $data_product[img_500];
                $product_options = $data_product[options];   
                $product_price = $data_product[shop_price];
                $product_qty = $data_product[qty]; 
                $product_cs_num = $data_product[order_cs];
                $product_cs_name = $this->get_order_cs2($product_cs_num);
            
                //  하단 테이블   
                $print_packs .= "<tr><td><img src=" . $product_img . " width=100 height=100></td>
                                <td align=center>" . $product_name . "</td>
                                <td align=center>" . $product_options . "</td>
                                <td align=center>" . $product_qty . "</td>
                                <td align=center>" . $product_price . "원</td>
                                <td align=center>" . $product_cs_name . "</td></tr>";   //  상품명 height='30'
            }
             $print_packs .= "</table><br><br></div>";
        }   
        return $print_packs;
    }
    ...
    // 1page에 100개씩 출력 
    function search_history_page()
    {
        global $connect, $idx, $deliv_all, $type, $product_name, $is_pack;


        // is_download=0, is_count=1
        $query = $this->get_query(0,1);
        ...
        $total_rows = $data[cnt];
		if ( $total_rows % 100 )
            $_tot_page = ((int)($total_rows / 100 )) + 1;
        else
            $_tot_page = $total_rows / 100;

        $_data = array();
		$_data['page']  = $_tot_page;
        $_data['total'] = $total_rows;
    ...
    {
        global $connect, $idx, $deliv_all, $type, $product_name, $page, $is_pack;
 
        debug("search_history 0" );
        //echo "aaaaa";
        //return;
        ...
        debug("search_history 111111" );
        
        $query = $this->get_query();

debug( "search_history:" . $query );
    ...
    // 0: 일반 조회, 1: 다운로드 조회
    // is_count: 0: 일반 1: 개수
    function get_query( $is_download = 0, $is_count=0)
    {
        global $connect,$idx, $deliv_all, $type, $product_name, $options, $price,$page,$is_pack;
	    ...
	    else
        {
		    if ( $is_count == 0 )
            {
                $query .= " order by a.pack,b.order_date, order_time limit $starter, 100";
            }
        }
    ...
    function DR02()
    {
        ...
        global $template, $page;
        global $start_date, $end_date, $confirm;
        global $trans_who, $status,$supply_code,$s_group_id,$shop_id,$group_id,$start_date,$end_date, $start_hour, $end_hour,$query_string;
        ...
        if ( $query_string )
            $query .= " and a.recv_name='$query_string' ";
    ...
    {   
        ...
        global $startHour, $endHour;  // 2014.7.15 추가
        ...
        $query = "select seq,pack,pack_lock
                    ...
                    , sum(qty) sqty
                    from orders 
                   where timestamp(collect_date, collect_time) >= '$start_date $startHour:00:00'
                     and timestamp(collect_date, collect_time) <= '$end_date $endHour:59:59'
                     and status=1
...
//debug("deliv_priority : " . $deliv_priority);
        ...
        $this->wrap_debug( "make_printable1: $query ",1);
        ...
        // print_enable_sort 테이블 생성..
        // pack, total_qty 의 정보 저장.. 
        // 9room은 order_date를 본다. - 황실장 요청사항 2014.11.25
        if ( _DOMAIN_ == "brownbunny" || _DOMAIN_ == "jucifaci" || _DOMAIN_ == "9room")
        {
    ...
    */
        ...
        $this->wrap_debug( "make_print q2: $query",1);
    ...
    function check_part_deliv2( $seq, $pack="", $qty_part_deliv, $use_app=false )
    {
        ...
        // 69로 시작하는 우편번호
        // 2014.11.21 
        if ( _DOMAIN_ == "dm" || _DOMAIN_ == "pimz")
        {
            if ( $pack )
                $query = "Select recv_zip from orders where pack=$pack";
            else
                $query = "Select recv_zip from orders where seq=$seq";
    
            $result = mysql_query( $query, $connect );
            $data   = mysql_fetch_assoc( $result );
            
            // 제주..
            if ( substr( $data[recv_zip],0,2) == "69" )
            {
                debug("$pack 제주 배송 안 함 " . $data[recv_zip] );
                return;    
            }  
        }
        
        
        // thedaze의 경우엔 공급처가 20002가 포함된 경우 부분배송 하지 않는다. 2014.11.10 14:28- JKRYU - 황실장요청
        /*
        if ( _DOMAIN_ == "thedaze" || _DOMAIN_ == "pimz")
        {
            if ( $pack )
                $query = "Select seq from orders where pack=$pack";
            else
                $query = "Select seq from orders where seq=$seq";

            debug( "부분 배송에 제외 $query");
            ...
            $query = "select count(*) cnt from order_products where order_seq in ( $str_seqs ) and supply_id='20002'";
            debug( "공급처 자사 부분 배송에 제외 $query");
            $result = mysql_query( $query, $connect );
            $data   = mysql_fetch_assoc ($result );
            $cnt2   = $data[cnt];
        ...
        if ( $cnt1 >= $qty_part_deliv )
        {
            ...
            if ( $cnt1 == $cnt2 )
            {
                ...
                if ( $data[tot] >= $qty_part_deliv )
                {
                    ...
                    // part 1. 미배송 주문의 처리..
                    //
                    // b.pack = $pack 에서 b.org_pack = $pack으로 수정 - jkryu 2014.11.5
                    $query = "select a.seq from orders a, print_enable b
                               where a.seq = b.order_seq and 
                                     a.order_cs not in (1,3,4,12) and
                                     (b.is_deliv_all is null or b.is_deliv_all = 0) and
                                     b.org_pack = $pack";
                    // order_cs가 2는 부분배송 - 부분배송건이 포함됨 오류가 있었음 - 수정 2014.11.7
                    $this->wrap_debug($query,1);
                    
                    $result = mysql_query($query, $connect );
                    $_zzz = "";		// 합포를 분리해야할 관리번호 리스트.
                    $_yyy = "";		// 새로운 합포 번호
                    $_cnt = 0;
                    while ( $data = mysql_fetch_assoc ( $result ) )
                    {
                    ...
                    // 굳이 합포 분리를 안 해준다. - 2013.7.25 - jkryu                  
                    // 전체 취소일 경우에만..부분 취소의 경우에는 기존대로 한다.
                    $query = "update orders set pack=$new_pack where pack=$pack and order_cs = 1";
                    $this->wrap_debug("[333]" . $query, 1);
                    //debug( "취소 pack:" . $query );
                    mysql_query( $query, $connect );
                    
                    // 대신에 해당 주문이 전체취소인지 부분 취소인지 확인하는 로직을 넣어야 함.
                    // 전체 상품수 == 취소 상품수 => 전체 취소
                    // 전체 상품수 != 취소 상품수 => 부분 취소
...
//                        echo "신규 주문 생성 $_new_seq / $data[order_seq] \n";
    ...
    function check_stock( $seq, $pack )
    {
        ...
        for( $i=0; $i < count($arr_result); $i++ )
        {
            ...
            foreach ( $arr_stock as $_stock )
            {
                ...
                debug(" $data[product_id]  seq: $seq / currnet stock: $_current_stock / stock1: $_stock[stock] / job stock: " . $arr_job_stock[ $data[product_id] ] . " pack stock: ".$arr_pack_stock[ $data[product_id] ] . " rev cnt: $reserve_cnt");
    ...
    function init_printable($use_app=false)
    {
        ...
        // print_enable에 값이 있을경우 print_enable_history에 값을 입력한다. 
        // 원래는 초기화를 하면 print_enable => print_enable_history로 입력이 됨 ( lock에서 오류가 발생하는 경우가 있음)
        // date: 2014-11-03
        $query = "select count(*) cnt from print_enable";
        $_result = mysql_query( $query, $connect );
        $_data   = mysql_fetch_assoc( $_result );
        if ( $_data[cnt] > 0 )
        {
            ...
            debug("[init_printable idx: $idx] 초기화 실패로 인해 make_printable에서 초기화");
            ...
            // make_printable_history에 값이 이미 있을 수 있다.
            $query = "delete from print_enable_history where idx='$idx'";
            debug( $query );
            mysql_query( $query, $connect );
        ...
        #debug("init_printable supply_id:" . $supply_id );
        #debug("init_printable supply_code:" . $supply_code);
  ...
  function notyet_count()
  {
    ...
    {
        global $connect, $trans_who, $shop_id, $supply_code, $status, $confirm, $warehouse, $key,$group_id,$s_group_id,$start_date,$end_date,$query_string;
        ...
        $query = "select count(distinct a.seq) cnt, sum(a.qty) s_qty from orders a, order_products b where a.seq=b.order_seq and ";
        //////////////////////////////////
        if ( $status == 1 ) // 정상
        {
        ...
        if ( $key == "pack" )
        {
            $query = "select if(a.pack<>null||a.pack<>0,a.pack, a.seq ) xx, count(*) cnt, sum(a.qty) s_qty 
                        from orders a, order_products b
            ...
            if(_DOMAIN_ == "box4u" || _DOMAIN_ == "ezadmin" )
            	echo number_format($rows)." (총상품수량:".number_format($tot_qty).")";
            else
            	echo number_format($rows);
        }
        else if ( $key == "total_products_cnt" )
        {
            $query = "select sum(b.qty) cnt 
                        from orders a, order_products b
                       where a.seq = b.order_seq";
            ...
            echo $data[cnt] ? number_format($data[cnt]) : 0;  
        }
        else
        {
            $result = mysql_query ( $query, $connect );
            $data = mysql_fetch_array ( $result );
            if(_DOMAIN_ == "box4u" || _DOMAIN_ == "ezadmin" )
            	echo number_format($data[cnt])." (총상품수량:".number_format($data[s_qty]).")";
            else
            	echo number_format($data[cnt]);
        }
        exit;      
    }
...
";
  ...
  function convert_title( $key )
  {
        switch ( $key )
        {
                ...
                case "seq_real":     return "주문번호"; break;
  ...
  function get_list2( $download_config, $dn_seqs="", $idx=""  )
  {
    ...
    function get_list_new( $download_config )
    {
        ...
        global $enable_sale,$group_id,$s_group_id,$start_date, $end_date, $sort;

        global $start_hour, $end_hour, $trans_corp,$query_string;
        ...
        if ( $download_config[pack_multiline] )
        {
            ...
            // box4u는 30067을 받거나 null, 
            if ( _DOMAIN_ == "box4u" )
            {
                if ( $trans_corp == 30067 )
                {
                    $query .= " and a.trans_corp = 30067 ";
                }
                else
                {
                    $query .= " and (a.trans_corp is null or a.trans_corp <> 30067) ";
                }
            }
            
            // 보류를 다운받지 않는다. 황실장 요청 2014.4.30
            if ( _DOMAIN_ == "soimall"|| _DOMAIN_ == "mizne" )
            {
                $query .= " and a.hold = 0 ";
            }
        ...
        if ( _DOMAIN_ != "ggstory" )
        {
            if ( _DOMAIN_ == "box4u" )
            {
                $query .= " order by c.location*1, c.location, a.seq, c.name";
            }
            else if ( $download_config[sortinfo] )
            {
                //$query .= " order by " . $download_config[sortinfo];
  ...
  function memo ( $data, $macro )
  {
        ...
        if ( $data[pack] )
        {
            ...
            while ( $data1 = mysql_fetch_array ( $result ) )
            {
                ...
                // box4u는 동일한 메모를 중복 출력한다.
                // 2014.5.21 box4u 성세일 요청사항
                if(_DOMAIN_ == "box4u" )
                {
                    $string .= $clean_memo;
                }
  ...
  */
  ...
  function get_order_data( $seq, $field )
  {
    ...
    // 최대 배송비 가져옴
    // 2014.5.02 최웅
    function max_trans_price( $data, $macro="" )
    {
        global $connect;        
        $seq = "";
		...
		if($macro)
		{
			if ( preg_match( "/[\[](.*)[\]]/", $macro, $matches ) )
			{
				$arr_macro = split( "\|\|", $matches[1] );
				$default_val = 0;
				foreach($arr_macro as $val)
				{
					$tmp_arr = split( "\:", $val );
					if(count($tmp_arr) == 1)
					{
						$default_val = $tmp_arr[0];
						continue;
					}
					
					if($tmp_arr[0] == $data[max_trans_price])
						return $tmp_arr[1];
				}
				if($default_val)
				 return $default_val;
    ...
    // 판매처별 배송지 설정 가능..
    function get_trasn_sender_info( $shop_id )
    {
        global $connect;   
    ...
    function is_island( $data )
    {
        ...
        if(in_array($zipcode, array(548906,409851,409919,513891,513893,535841,535897,535924,535934,690823,690824,695962,697110,697800,697830,699904,699937,690170,695911,697703,535862,535861,535811,535832,535835,535838,535873,535871,539911,539913,539915,539917,539918,539914,537843,535852,535851,409912,537849,537845,537841,535914,535912,535917,535918,550270,537852,537921,537818,537922,537832,537834,537831,537835,537813,548992,548991,548994,537902,537809,537903,690029,690032,556851,650922,799811,699701,699921,697390,697857,697819,697860,573813,690801,690800,355848,409893,535893,799821,690011,690012,690021,690022,690031,690041,690042,690043,690061,690062,690071,690072,690073,690081,690082,690191,690192,690231,690232,690241,690242,690747,537824,537823,535891,535896,417911,417912,417913,417931,417922,417921,535942,535943,535842,535843,535845,535847,535840,535844,697827,697829,699947,699946,699933,690714,690750,690110,690819,690727,695983,690826,690844,690741,690751,690778,695967,695968,695952,695941,695942,690808,690736,690121,695919,695917,695918,695947,695943,695922,695926,695928,695932,690200,690839,690785,690725,690719,690765,573815,573955,573816,690796,697705,697704,697805,697846,697080,697850,699908,699907,699906,697856,690729,690789,690708,690841,690805,690790,690718,690090,690812,537901,690846,618410,664250,650931,650932,650933,650934,650930,650941,650944,650945,650911,650912,650914,650915,650910,650833,650835,650921,650923,650924,650925,650926,650927,650920,664270,656836,618430,618450,618420,799822,799820,799812,799810,799801,799802,799805,799804,799800,530440,513892,513890,556841,556843,556844,556847,556848,556840,556852,556853,409911,409913,409910,409831,409832,409833,409830,409842,409841,409840,409882,409883,530430,556854,556850,556832,556835,556836,556838,556839,556830,536929,548931,548933,548932,548936,548930,548902,548909,548894,548941,546908,535863,535860,535813,535816,535831,535834,535833,535836,535837,535830,535872,537815,537814,537816,537817,536935,548990,537904,537905,537909,535870,535921,535922,535923,535925,535920,535823,535824,535806,539912,539916,537907,548993,537900,537842,536928,537844,537847,539919,539910,537846,537848,535850,535931,535932,535933,535935,535936,535930,535915,535913,535911,535919,535916,535910,537840,537851,537853,537850,537920,537833,537836,537830,409852,409853,409850,409891,409892,537821,537822,537825,537826,537820,535892,535894,535895,535898,535890,535881,535882,535883,535884,535885,535880,535941,535926,535940,539832,417923,417920,417910,417932,417933,417930,409890,697810,695975,535846,690030,530847,355846,697809,695974,690230,697815,699942,697801,697802,530145,535805,697811,699902,556842,697804,695973,690020,573818,695963,697812,556849,699905,357941,690728,690784,690770,690828,690829,690764,690739,690817,690816,690815,690814,690813,690774,690722,690737,690726,690724,690703,690700,690140,690150,690163,690740,690162,690161,690775,690820,690843,690821,690822,695980,690130,690827,690825,690788,690851,690701,690732,690776,690731,690787,690730,690780,690835,690836,690833,690831,690832,690830,690834,690734,690777,690779,695961,695966,695969,695964,695960,695951,695981,579911,579912,579913,579914,579915,579910,695944,695945,695946,695948,695949,695940,695921,695923,695924,695925,695927,695930,695929,695931,695933,695934,695920,690769,690838,690837,690781,690100,690600,690610,356878,355842,355845,355847,697831,697864,699903,699900,697330,690705,690720,343852,695977,695972,695978,695979,695976,695982,695970,690210,690003,690762,690772,690804,690803,690802,690180,690760,690706,690707,690709,690744,690840,690742,690743,690842,690755,690807,690806,690847,690712,690220,690735,690810,690811,537906,690715,690710,690711,690850,690809,699912,697826,697828,697370,699941,699949,699943,699944,699945,699948,699940,699936,699931,699932,699934,573811,573812,699935,699702,699930,697320,697825,697350,697832,697833,697836,697839,697706,697838,697070,690783,690122,690756,690773,690767,695901,695902,695903,695904,695705,695792,695791,695905,695906,695907,695908,695909,695796,695793,695910,695789,695912,695913,695914,695916,695794,695915,695795,695900,690786,690818,690723,690738,690721,690717,690766,573814,573817,573819,573810,697834,697837,697700,697835,697806,697020,697301,697050,697821,697820,697130,697807,697808,697840,697842,697841,697844,697010,697843,697847,697100,697845,697848,697853,697852,697701,697849,697854,697851,699901,697013,697040,697855,695965,695950,699925,699922,699926,699923,699924,699920,697380,697360,697014,697120,697707,697858,697011,697012,697060,697861,697822,697859,699911,699913,699914,699915,699916,699910,697310,697340,697824,697030,697090,697862,697863,697823,697600,690050,690704,690771,690782,695971,690921,690070,556856,690713,697818,697817,690120,690716,697816,697803,556845,690733,530800,690010,690080,690040,697813,690160,690190,690240,799823,690060,697814,656876,799813,799803,656835,618440,650946,650913,650916,556855,556831,556834,556837,556846,555300,409881,409880,400460)))
        {
        	return "(도서지역)";   
        }                    
        else
    ...
    function get_product_location( $data )
    {
        ...
        debug( "[get_product_location]" . $query . "/" . $data[location] );
        ...
        else
        {
            ...
            else
            {
                ...
                {  
                    ...
                    case "max_trans_price":
                    	$string .= $this->max_trans_price( $data );
                    	break;
                    
                    case "sender_address1":
                        $arr_trans_info = $this->get_trasn_sender_info( $data[shop_id] );                        
                        $string = $arr_trans_info['trans_sender_add1'] ;                        
                        break;
                    case "sender_address2":
                        $arr_trans_info = $this->get_trasn_sender_info( $data[shop_id] );                        
                        $string = $arr_trans_info['trans_sender_add2'];                        
                        break;
                    case "sender_tel":
                        $arr_trans_info = $this->get_trasn_sender_info( $data[shop_id] );
                        $string = $arr_trans_info['trans_sender_tel'];
                        break;
                    case "sender_zip":
                        $arr_trans_info = $this->get_trasn_sender_info( $data[shop_id] );
                        $string = $arr_trans_info['trans_sender_zip'];
                        break;
                    case "sender_name":
                        $arr_trans_info = $this->get_trasn_sender_info( $data[shop_id] );
                        $string = $arr_trans_info['trans_sender_name'];
                        break;    
                    case "recv_address":
                        ...
                        {
                            $string .= strip_tags( str_replace( $search,"", $arr_trans_info[trans_address] ) );
                        }
                        else
                        {
    						...
    						$string .= strip_tags( str_replace( $search,"", $data[$key]) );
					    }
                        break;
                    case "order_name":	                
                        $search  = array('<', '>', '  ');
...
*/
  ...
  function get_total_price( $data )
  {
    ...
    function max_weight ( $data )
    {
        ...
        if ( _DOMAIN_ == "polotown")
            return $this->bcround( $_data[max_weight],3);
        else
    ...
    function total_origin( $data )
    {
        ...
        if ( $_data[pack] )
        {
            ...
            $query = "select a.product_id,a.origin, b.qty
                    from products a, (select product_id,qty from order_products where order_seq in ( $str_seqs )  and order_cs not in (1,2) ) b
                    where a.product_id=b.product_id";
        }
        else
        {
            ...
            $query = "select a.product_id,a.origin, b.qty
                        from products a, (select product_id,qty from order_products where order_seq=$data[seq]  and order_cs not in (1,2) ) b
                        where a.product_id=b.product_id";
        ...
        while ( $data   = mysql_fetch_assoc( $result ) )
        {
            $total_origin += $data[origin] * $data[qty];    
        }
        
        if (_DOMAIN_ == "polotown" )
            $total_origin = $this->bcround( $total_origin,3);
    ...
    function total_weight( $data )
    {
        ...
        if ( _DOMAIN_ == "polotown")
            $total_weight = $this->bcround( $total_weight,3);
  ...
  function get_pack_datas ( $pack, $arr_macro, $macro_case, $separator='', $max='' )
  {
        ...
        if ( _DOMAIN_ == "suvin" || _DOMAIN_ == "gerio" || _DOMAIN_ == "dabagirl2" )
        {
            $query = "select * , seq as product_seq
		...
		else if ( _DOMAIN_ == "joongwon" )
        {
            $query = "select * , seq as product_seq,sum(qty) qty
                    from order_products , products
                   where order_products.product_id = products.product_id
                     and order_products.order_seq in ( $seqs ) 
                     and order_products.order_cs not in (1,2,3,4) 
