...
class class_DG00 extends class_top
{
    //###############################
    // 메인 화면
    //###############################
    function DG00()
    {
        ...
        {
            // 전체 쿼리
            $data_all = $this->get_DG00($f, &$total_rows, &$sum_arr);
    ...
    // 데이터 조회
    //###############################
    function get_DG00($f, &$total_rows, &$sum_arr)
    {
...
debug("주문재고할당 불러오기 : " . $query);
        ...
        while( $data = mysql_fetch_assoc($result) )
        {
            if( $data[pack_seq] == 0 || $old_pack_seq <> $data[pack_seq] )
            {
                ...
                // 이전 pack_qty == 1 이면 단품주문으로 제외
                if( $pack_qty == 1 )  array_pop($data_all);
            ...
            // 출력 정보 가져오기
            $temp_arr = $this->get_DG00_data_arr($data, $f, $supply_info);
    ...
    function get_DG00_query()
    {
        ...
        // 시간 설정
        if( $start_hour == "00:00:00" && $end_hour == "23:59:59" )
            $query .= "  and a.collect_date between '$start_date' and '$end_date' ";
        else if( $start_date == $end_date )
            $query .= "  and a.collect_date='$start_date' and a.collect_time between '$start_hour' and '$end_hour' ";
        else
            $query .= "  and timestamp(a.collect_date, a.collect_time) between '$start_date $start_hour' and '$end_date $end_hour' ";
        ...
        if ( $query_str )
        {
            ...
            {
                case "name":
                    $query .= " and c.name like '%$query_str%' ";
                    break;
                case "options":
                    $query .= " and c.options like '%$query_str%' ";
                    break;
                case "product_id":
                    $query .= " and c.product_id = '$query_str' ";
                    break;
            }
        }
    ...
    function save_file_DG00()
    {
        ...
        // 전체 쿼리
        $data_all = $this->get_DG00($f, &$total_rows, &$sum_arr, 1);

        $data_all[] = $sum_arr;
        $fn = "request_stock_" . date("Ymd_His") . ".xls";
        $this->make_file_DG00( $data_all, $fn, $f );
    ...
    // 파일 생성
    //###############################
    function make_file_DG00( $data_all, $fn, $f )
    {
        global $connect;
    ...
    // 출력 row data 생성하기
    //*****************************
    function get_DG00_data_arr($data, $f, $supply_info)
    {
        global $connect;
        global $work_no, $order_status;
        global $stock_info, $status_qty1, $status_qty7, $assign_qty;
        ...
        foreach( $f as $f_val )
        {
            ...
            {
                $name_str = "<span class='pname_style'>" . ($data[b_gift] ? "<img src='images/gift2.gif'>" : "") . htmlspecialchars($product_info[$f_val[field_id]], ENT_QUOTES) . "</span>";
                $option_str = "<span class='option_style'>" . htmlspecialchars($product_info[$f_val[field_id]], ENT_QUOTES) . "</span>";

                if( $f_val[field_id] == "product_name" )
                    $temp_arr[$f_val[field_id]] = $name_str;
                else if( $f_val[field_id] == "options" )
                    $temp_arr[$f_val[field_id]] = $option_str;
                else if( $f_val[field_id] == "name_options" )
                    $temp_arr[$f_val[field_id]] = $name_str . $option_str;
                else
                    $temp_arr[$f_val[field_id]] = $product_info[$f_val[field_id]];
                continue;
            }
            ...
            if( $f_val[field_id] == "seq" )
                ...
                $temp_arr[$f_val[field_id]] = "<span class='name_style'>" . ($data[a_pack_lock] ? "<img src=./images/lock.png> " : "") . $this->popupcs($data[a_seq], htmlspecialchars($data[a_recv_name], ENT_QUOTES)) . "</span>";
