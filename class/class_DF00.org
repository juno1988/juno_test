...
// 59 : 엘투
// 65 : 싸이마켓
// 97 : 꿈이있는세상 가비아 (10045)와 동일함
// 66 : 샵링커
// 68 : 메이크샵
// 80 : 오가게 - 2007.3.8 - jk.ryu 
// 76 : 오가게(망고스틴) - 2007.10.1 - jk.ryu 
// 
//
class class_DF00 extends class_top 
{
   ...
   function search()
   {
      ...
      $limit_option = 0;
      $no_cancel = 2;
      $result = class_D::get_order_list( $total_rows, $limit_option, $search_date, $no_cancel );
...
";
   ...
   function download2(  )
   {
      ...
      switch ( $format )
      {
	    ...
	    /*
            ...
            $this->build_excel( $workbook, $download_items, $result, $header );
	    */
	 ...
	 if ( substr($data[order_id],0,1) == "C" ) continue;
	    ...
	    //for ( $z=0; $z< count($download_items); $z++)
            foreach ( $download_items as $key=>$value )
	    {
	 ...
	 //for ( $z=0; $z<count ($download_items); $z++)
         ...
         {
	       // list ( $key , $value ) = $download_items[$z];
               $buffer .= $this->get_data( $data, $key, $i_cnt, $value );
               $buffer .= $division;
         }
   ...
   // 값을 선택
   // $order값은 count
   function get_data( $data, $key, $order_cnt = 0 , $value="")
   {
      global $shop_id, $connect;

      $this->get_transinfo ( $data[trans_corp], $data[shop_id], &$trans_name, &$trans_code, &$trans_no );
      ...
      switch ( $shop_xp )
      {
         // daum일 경우 supply의 주소를 가져와야 함
         case 3:
            $query = "select zip1, zip2, address1, address2, tel from userinfo where code = '$data[supply_code]'";
	...
	case "user_defined7":
	case "user_defined8":
		list( $header, $value ) = split(":", $value );
		return $value;
		break;
        case "recv_name":
		if (_DOMAIN_ == "mangosteen" and $data[shop_id] == '10015')
			return "전의택";
		else
			return $data[recv_name];
		break;
	case "130":
		return 130;
		break;
	...
	case "hana_id":
        ...
        case "cy_order_date":
            ...
            break;
        case "order_id":
	    if ( _DOMAIN_ == "sevar" && $data[shop_id] == 10076 )
                return $data[order_id]."-".$data[code1];
	    else
	        return $data[order_id];
            break;
        case "cy_collect_date":
        ...
        case "halfclub_code":
           switch ( _DOMAIN_ )
           {
               case "jyms": return "A0699"; break;
               case "ezadmin": return "A0699"; break;
	...
	case "no":
	   ...
	   break;
        case "cj_order_id" :
           $arr = array ( "-" );
           return str_replace($arr, "", $data[order_id]);
        break;
        case "collect_date2";
           $arr = array ( "-" );
           return str_replace($arr, "", $data[collect_date]);
        break;
        case "onket_order_id":
           return "=TEXT(" . $data[order_id] . ",\"0\")";
           //return chr(39) . "$data[order_id]";
        break;
        case "onket_arrive_date":
           $arrive_date = str_replace("-" , "", date('Ymd', strtotime('1 day')) );
           return $arrive_date; 
           // return "=TEXT(" . $arrive_date . ",\"0\")";
           //return "$arrive_date";
        break;
        case "onket_deliv_way":       // 배송 방법
           // return "=TEXT(\"00\",\"00\")";
           return "00";
        break;
        case "deliv_way":       // 배송 방법
           return "택배 ";
        break;
        case "arrive_date":     // 도착 예정일
           return date('Y-m-d', strtotime('1 day')) . " ";
        break;
        case "trans_name":      // 운송회사 이름
           return $trans_name;
        break;
        case "trans_code":      // 운송회사코드 
           return $trans_code ? $trans_code : $trans_name;
        break;
        case "order_date2":  // 배송 의뢰일 
          return str_replace( "-", "", substr( $data[order_date],0,10 ));
        break;
        case "trans_date":  // 배송 예정일
          return str_replace( "-", "", ( substr( $data[trans_date] ? $data[trans_date] : $data[trans_date_pos], 0, 10 )));
        break;
        case "trans_date_pos":  // 배송일
          return str_replace( "-", "", ( substr( $data[trans_date_pos] ? $data[trans_date_pos] : $data[trans_date], 0, 10 )));
        break;
        case "_trans_date_pos":  // 배송일
          return substr( $data[trans_date_pos] ? $data[trans_date_pos] : $data[trans_date], 0, 10 );
        break;
        case "trans_no":        // 운송장 번호
        ...
        break;
        case "return_zip":
           return $return_data[zip1] . "-" . $return_data[zip2];
        break;
        case "return_address":
           return $return_data[address1] . " " . $return_data[address2];
        break;
        case "return_tel1":
           return $return_data[tel];
        break;
        case "return_tel2":
           return $return_data[mobile];
        break;
        case "price_order_id":
           return $data[code1]; 
        break;
        ...
        case "미적용":
           return "미적용";
        break; 
        case "interpark_open_order_id":
           return $data[order_id]; 
        break;
        case "interpark_order_id":
           return "=\"" . $data[order_id] . "\""; 
        break;
        case "v":
           return "v";
        ...
        break;
        case "product_name":
	   return class_product::get_product_name( $data[product_id] );
           // return str_replace( array( "\<", "\>", "\<\<" , "\>\>"), "", $data[product_name]);
	break;
        default:
           $val = $data[$key] ? $data[$key] : "";
           return str_replace( array( $division,"\r", "\n", "\r\n","\t"), "", $val );
        break;
      } 
   }
